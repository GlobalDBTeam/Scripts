...
set linesize 200
col file_name format a90 heading "Datafile Name"
col allocated_mb format 999999.99;
col used_mob format 999999.99;
col free_mb format 999999.99;
col tablespace_name format a20;
SELECT SUBSTR (df.NAME, 1, 90) file_name, dfs. tablespace_name, df.bytes / 1024 / 1024 allocated_mb,
...
set line 300;
col "RMAN_BACKUP_TAG" format a40
col "DEVIC" format a6;
col "L" format 9;
...
col "ARC_BACKUP_MB" format 999999.99; 
...
FROM  gv$system_event w, (select inst_id, sum(time_waited) twt  from gv$system_event
  where time_waited>0
   AND event NOT IN ('Null event', 'client message', 'rdbms ipc reply', 'smon timer', 'rdbms ipc message', 'PX Idle Wait', 'PL/SQL lock timer', 'file open', 'pmon 

timer', 'WMON goes to sleep',
...
WHERE  w.inst_id = tw.inst_id and w.time_waited>0
 ...
 and w.event NOT IN ('Null event', 'client message', 'rdbms ipc reply', 'smon timer', 'rdbms ipc message', 'PX Idle Wait', 'PL/SQL lock timer', 'file open', 'pmon 
